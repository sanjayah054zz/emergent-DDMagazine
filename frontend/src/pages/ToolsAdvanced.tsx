...
const vehicles = [
  { id: "tenere", name: "Yamaha Ténéré 700", type: "DUAL-SPORT ADVENTURE", engine: "689cc CP2 Parallel-Twin (72.4 hp)", weight: "205 kg (wet)", price: "$10,799 USD", rating: "8.7 / 10", economy: "23.8 km/L", image: "https://images.unsplash.com/photo-1588756681780-9d5859fc2ca0?crop=entropy&cs=srgb&fm=jpg&ixlib=rb-4.1.0&q=85" },
  { id: "cfmoto", name: "CFMoto 450 MT", type: "LIGHTWEIGHT MID-ADVENTURE", engine: "449cc 270° Parallel-Twin (44 hp)", weight: "190 kg (wet)", price: "$6,499 USD", rating: "9.2 / 10", economy: "26.5 km/L", image: "https://images.pexels.com/photos/27662214/pexels-photo-27662214.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940" },
  { id: "miata", name: "Mazda MX-5 Miata ND3", type: "LIGHTWEIGHT SPORTS ROADSTER", engine: "2.0L Skyactiv-G 4-Cyl (181 hp)", weight: "1,061 kg", price: "$28,985 USD", rating: "8.1 / 10", economy: "14.5 km/L", image: "https://images.pexels.com/photos/9846051/pexels-photo-9846051.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940" },
  { id: "volvo-ex30", name: "Volvo EX30 Twin Motor", type: "COMPACT ELECTRIC CROSSOVER", engine: "Dual-Motor AWD (422 hp)", weight: "1,943 kg", price: "$46,195 USD", rating: "9.0 / 10", economy: "17.5 kWh/100 km", image: "https://images.unsplash.com/photo-1617727553401-3ec4e92f32a5?crop=entropy&cs=srgb&fm=jpg&ixlib=rb-4.1.0&q=85" },
  { id: "rav4-hybrid", name: "Toyota RAV4 Hybrid", type: "HYBRID DAILY SUV", engine: "2.5L Hybrid AWD (219 hp)", weight: "1,720 kg", price: "$33,295 USD", rating: "9.4 / 10", economy: "17.0 km/L", image: "https://images.pexels.com/photos/29550422/pexels-photo-29550422.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940" },
  { id: "civic-type-r", name: "Honda Civic Type R", type: "PERFORMANCE HOT HATCH", engine: "2.0L Turbo 4-Cyl (315 hp)", weight: "1,430 kg", price: "$45,895 USD", rating: "8.6 / 10", economy: "10.2 km/L", image: "https://images.unsplash.com/photo-1590509335747-05b072c29a98?crop=entropy&cs=srgb&fm=jpg&ixlib=rb-4.1.0&q=85" },
  { id: "bronco-badlands", name: "Ford Bronco Badlands", type: "TRAIL-READY OFF-ROADER", engine: "2.7L Twin-Turbo V6 (330 hp)", weight: "2,263 kg", price: "$51,990 USD", rating: "7.8 / 10", economy: "7.7 km/L", image: "https://images.unsplash.com/photo-1764053001030-35e9e6ee3c21?crop=entropy&cs=srgb&fm=jpg&ixlib=rb-4.1.0&q=85" },
  { id: "mclaren-artura", name: "McLaren Artura", type: "HYBRID PERFORMANCE SUPERCAR", engine: "3.0L Twin-Turbo V6 + E-Motor (690 hp)", weight: "1,498 kg", price: "$237,500 USD", rating: "7.4 / 10", economy: "10.2 km/L", image: "https://customer-assets-lxgj4vgw.emergentagent.net/job_track-street-mode/artifacts/l5uvtuts_The%20Ones%20Who%20Rise%20_%20Departures%20Magazine.jpg" },
];
      ...
      <main className="tools-main" data-testid="tools-main">
        ...
        <section className="interactive-section tools-section" id="garage" data-testid="tools-garage-section">
          ...
          <div className="garage-layout"><div className="garage-controls" data-testid="tools-garage-controls"><div className="control-field"><label htmlFor="tools-vehicle-a" data-testid="tools-vehicle-a-label">VEHICLE A</label><div className="select-wrap"><select id="tools-vehicle-a" value={vehicleA} onChange={(event) => setVehicleA(event.target.value)} data-testid="tools-vehicle-a-select">{vehicles.map((vehicle) => <option value={vehicle.id} key={vehicle.id}>{vehicle.name}</option>)}</select><ChevronDown size={16} /></div></div><div className="control-field"><label htmlFor="tools-vehicle-b" data-testid="tools-vehicle-b-label">VEHICLE B</label><div className="select-wrap"><select id="tools-vehicle-b" value={vehicleB} onChange={(event) => setVehicleB(event.target.value)} data-testid="tools-vehicle-b-select">{vehicles.map((vehicle) => <option value={vehicle.id} key={vehicle.id}>{vehicle.name}</option>)}</select><ChevronDown size={16} /></div></div><Button className="outline-button" onClick={() => setShowCompare((show) => !show)} data-testid="tools-compare-button">{showCompare ? "HIDE COMPARISON" : "COMPARE MACHINES"}<SlidersHorizontal size={16} /></Button><div className="garage-note" data-testid="tools-garage-note"><ShieldCheck size={16} /><span>Specs verified by the Daily Driver test bench.</span></div></div>
            {showCompare && <div className="comparison-sheet" data-testid="tools-comparison-sheet"><div className="comparison-head"><span className="eyebrow" data-testid="tools-comparison-label">LIVE COMPARISON</span></div><div className="vehicle-columns">{selectedVehicles.map((vehicle, index) => <div className="vehicle-column" key={vehicle.id} data-testid={`tools-vehicle-${index === 0 ? "a" : "b"}-column`}><div className="vehicle-column-title"><span>{vehicle.type}</span><h3 data-testid={`tools-vehicle-${index === 0 ? "a" : "b"}-name`}>{vehicle.name}</h3></div><img className="comparison-vehicle-image" src={vehicle.image} alt={vehicle.name} data-testid={`tools-vehicle-${index === 0 ? "a" : "b"}-image`} />{[["ENGINE", vehicle.engine], ["WEIGHT", vehicle.weight], ["PRICE", vehicle.price], ["COMMUTE RATING", vehicle.rating]].map(([label, value]) => <div className="spec-cell" key={label}><span className="spec-label">{label}</span><strong>{value}</strong></div>)}<div className="mini-spec-row"><span>ECONOMY</span><strong>{vehicle.economy}</strong></div></div>)}</div></div>}
          </div>
        </section>
